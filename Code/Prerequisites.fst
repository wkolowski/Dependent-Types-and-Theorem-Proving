...
type intOrString2 =
    | Left2  : int    -> intOrString2
    | Right2 : string -> intOrString2
...
// Records can also be opened for easier access.
...
type stuff2 =
    | Mkstuff2 : int -> string -> bool -> stuff2

// We have to implement all the projections... that's not nice.
let num2 (Mkstuff2 i _ _) = i
let str2 (Mkstuff2 _ s _) = s
let bl2  (Mkstuff2 _ _ b) = b

// However, if we give names to the constructor arguments, F* will generate
// the projections for us for free.
type stuff3 =
    | Mkstuff3 : (num : int) -> (str : string) -> (bl : bool) -> stuff3

let autogenerated_projection : stuff3 -> int =
    Mkstuff3?.num

// Note that in the definition of 'stuff2' above, the constructor has type
// 'int -> string -> bool -> stuff2'. Arrows associate to the right,
// so this should be read as 'int -> (string -> (bool -> stuff2)))'.
...
// The same thing can be expressed with the type 'int * string * bool -> stuff2'.
...
let curry (f : int * string * bool -> stuff2) : int -> string -> bool -> stuff2 =
    fun i s b -> f (i, s, b)

let uncurry (f : int -> string -> bool -> stuff2) : int * string * bool -> stuff2 =
    fun p ->
        match p with
        | (i, s, b) -> f i s b
