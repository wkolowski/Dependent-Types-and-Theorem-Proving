...
\begin{itemize}
	\item Dependent types are types that can depend on values.
	\item In dependently typed languages there is a universe -- a type whose elements are themselves types.
...
\begin{itemize}
	...
	\item To create a dependent pair, we write \m{(x, y)} where \m{x} is of type \m{a} and \textbf{\m{y} is of type \m{b x}}.
	\item To use a pair \m{p :\ (x :\ a) * b x}, we use projections -- we have \m{fst p :\ a} \textbf{and \m{snd p :\ b (fst p)}}.
...
\begin{frame}{Dependent products, Pi type and multiplication}
\begin{itemize}
	\item The dependent function type also goes by two other names: \textbf{the dependent product type and the Pi type}.
	\item \textbf{There's also a different notation}: \m{(x :\ a) -> b x} is sometimes written as $\displaystyle \prod x :\ a, b(x)$ or $\displaystyle \prod_{x :\ a} b(x)$ or similar.
\end{itemize}
\end{frame}
